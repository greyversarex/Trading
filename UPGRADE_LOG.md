...
## Sub-phase 3.1 — Реальный поток данных Binance по WebSocket

**Новое**
- `src/backend/binance_ws_feed.py` (`BinanceWebSocketFeed`): асинхронный клиент
  комбинированного klines-стрима Binance (`wss://stream.binance.com:9443/stream`,
  имена `{symbol}@kline_{tf}`). Методы `start/stop/_on_message`, авто-переподключение
  с экспоненциальным backoff (cap = `ws_reconnect_max_delay_sec`), heartbeat через
  ping/pong библиотеки `websockets`. Обновляет свечи in-place в
  `scanner.symbol_data`; колбэк рыночного обновления и перерасчёт структуры —
  только по ЗАКРЫТОЙ свече (`k.x == True`).
- `binance_scanner.py`: поле `ws_feed`; в `start()` при `CONFIG.data.use_websocket`
  поднимается WS-фид, иначе — REST-поллинг. При ошибке запуска WS — фаллбэк на
  REST. `stop()` корректно закрывает WS. Исправлено: `run_continuous_scan`
  в `main.py` больше не запускает REST-поллинг параллельно с WS.
- `config.py` (DataConfig): `use_websocket=True`, `ws_reconnect_max_delay_sec=30.0`,
  `ws_heartbeat_interval_sec=30.0`.
- `tests/test_binance_ws_feed.py` — 9 тестов: имена стримов/URL, маппинг пары,
  экспоненциальный backoff с ограничением, добавление новой свечи vs обновление
  текущей, колбэк только по закрытой свече, игнор неизвестных символов/ТФ,
  REST-фаллбэк при выключенном WS, запуск ws_feed при включённом WS.

**Серверный `/ws` (браузер↔сервер) не затронут** — это отдельный исходящий
клиент Binance.

**Проверка**
- `python -m pytest tests/test_binance_ws_feed.py -v` -> 9 passed.
- `python -m pytest -q` -> 115 passed.
- `python run.py`: чистый старт; инициализация 50 символов / 814 структур
  (исторические данные через REST), затем live-обновления через WS.

## Sub-phase 3.2 — Настраиваемая панель UI + визуализация

**Бэкенд**
- `src/backend/settings_store.py` — белый список настраиваемых параметров
  (`SETTINGS_SCHEMA`) поверх `CONFIG`. Валидация по типу/диапазону, атомарное
  применение, персист в `data/ui_settings.json`, загрузка на старте.
- `main.py`:
  - `GET /api/settings` — схема + текущие значения для построения UI.
  - `POST /api/settings` — валидирует, применяет к `CONFIG` в рантайме,
    сохраняет; синхронизирует глобальный `ml_score_threshold`; 400 при
    некорректном значении.
  - `POST /api/restart-scan` — перезапуск активного сканирования с новыми
    настройками.
  - `startup`: загрузка сохранённых настроек, синхронизация порога ML.
  - `/api/ml-status`: добавлено поле `ml_score_distribution` (10 бинов 0–1,
    кольцевой буфер последних 1000 ml_score).

**Фронтенд (`static/index.html`)**
- Сворачиваемая панель «Настройки»: слайдеры/тумблеры по категориям
  (данные/производительность, детекция, паттерны, свечи, ML). Кнопки
  «Сохранить» и «Сохранить и пересканировать».
- Бейдж ML-релевантности у каждого совпадения (красный <50% / жёлтый /
  зелёный ≥80%).
- Мини-гистограмма распределения ML-оценок в блоке «Статистика»
  (обновление каждые 15 с) с отметкой текущего порога.

**Контракт**: только новые поля/маршруты, обратная совместимость сохранена.

**Проверка**: `python run.py` — чистый старт; GET/POST `/api/settings`,
`/api/restart-scan`, `/api/ml-status` отвечают 200; валидация диапазонов
возвращает 400; UI рендерится без ошибок в консоли.

## Sub-phase 3.3 — Оптимизация производительности

**Кэширование структур**
- `_candle_hash`: теперь хэшируется весь ряд close-цен (а не 3–4 точки), чтобы
  не пропустить изменения в середине окна (иначе кэш отдал бы устаревшую
  структуру). Кэш в `_update_structure` управляется флагом
  `CONFIG.data.cache_structures` (можно отключить).

**Конкурентность**
- `_update_all_candles`: размер батча = `CONFIG.data.max_concurrent_symbol_tasks`
  (было жёстко 10).
- Новый `_update_all_structures_async`: параллельный пересчёт структур по
  символам через пул потоков (`asyncio.to_thread`) батчами того же размера.
  Расчёт CPU-bound (numpy/scipy освобождают GIL). Группировка по символу
  исключает гонки за общий объект. Используется при инициализации сканера.

**Конфигурация** (`DataConfig`): `max_concurrent_symbol_tasks=10`,
`cache_structures=True`.

**Тесты** (`tests/test_performance.py`, 6 шт.): пропуск пересчёта при неизменных
свечах, пересчёт при изменении, отключение кэша флагом, детекция изменения в
середине ряда хэшем, параллельный пересчёт по всем символам, согласованность
параллельного и последовательного результатов.

**Проверка**: `python -m pytest -q` -> 121 passed; `python run.py` — чистый старт.

## Sub-phase 3.4 — Повышение recall: cup_and_handle и triple_top

**Тройная вершина (асимметричные пики)**
- `_candidate_triple_top`: при глубоком нэклайне (depth > 0.15 диапазона)
  допуск равенства пиков ослабляется с `triple_top_tolerance` (0.035) до
  `triple_top_asymmetry_tolerance` (0.06). Снижен штраф за асимметрию
  расстояний между пиками — приоритет отдаётся факту чёткого пробоя нэклайна.

**Чаша с ручкой («чаша без ручки»)**
- Добавлен `_candidate_cup_fallback`: строит чашу по максимумам close-цен,
  когда основной детектор не нашёл кандидата. Правая кромка определяется как
  пик восстановления в окне `[0.6 .. 1.4]·span` после дна (не первое
  пересечение порога 0.97 — давало ложную асимметрию; не глобальный максимум
  хвоста — захватывал пробой). Ручка опциональна (`relaxed_handle_retrace_max`
  = 0.7); при её отсутствии паттерн всё равно подтверждается.
- Защиты от ложных кандидатов: левая кромка `li ≤ 0.40n`, правая `ri ≥ 0.55n`,
  расхождение кромок < 0.10·диапазона, симметрия ≥ 0.3.

**Конфигурация** (`PatternConfig`): `triple_top_asymmetry_tolerance = 0.06`,
`cup_and_handle_relaxed_handle_retrace_max = 0.7`.

**Переобучение и валидация**
- `scripts/generate_synthetic.py` (seed=42, 100/класс, 500 шума) →
  `scripts/train_ml.py` → проверка на синтетике.
- Каузальный recall (первичный тип == целевой): **triple_top 91%**
  (цель ≥75% — выполнено), **cup_and_handle 70%** (цель ≥65% — выполнено).
- **ML FPR = 0.00%** на 200 шумовых образцах (цель ≤1% — выполнено). Сырой
  (без ML) FPR ~45%, ML-фильтр полностью отсекает ложные подтверждения.

**Тесты** (`tests/test_missing_patterns.py`, +3): наличие новых параметров
конфигурации; распознавание «чаши без ручки» (fallback); распознавание
тройной вершины с неравными пиками (ослабленный допуск асимметрии).

**Проверка**: `python -m pytest -q` -> 124 passed; `python run.py` — чистый старт.
**Контракт REST/WS**: без изменений (только настройки детекции).

## Code review (architect) — исправления

После архитектурного ревью Фазы 3 устранены три замечания:
- **WS-фид (фаллбэк, высокий приоритет):** `BinanceWebSocketFeed.start()` теперь
  дожидается первого успешного подключения (до `ws_startup_timeout_sec = 10 с`) и
  возвращает `bool`. При неуспехе фоновая задача останавливается и сканер
  детерминированно переходит на REST-поллинг (раньше старт «успешен» даже без
  связи → молчаливый режим без данных).
- **Внеочередные свечи:** `_apply_candle` игнорирует сообщения со `open_time`
  меньше последней свечи (раньше старая свеча перезаписывала актуальную).
- **Тихие ошибки потоков:** `_update_all_structures_async` логирует исключения
  из `asyncio.gather` по символам (раньше отбрасывались молча).

**Тесты** (+2): игнорирование устаревшей свечи; обновлён фейк WS-старта под
новый bool-контракт. `python -m pytest -q` -> **125 passed**.
